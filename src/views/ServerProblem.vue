...
export default {
  ...
  data() {
    return {
      header: 'Технические неполадки',
      body: 'К сожалению на сервере технические неполадки, '
                    + 'мы приносим извинения и просим вас зайти позже ',
    };
  },
  methods: {
    load() {
      if (this.SERVER_PROBLEM) {
        this.$loadScript('https://cdnjs.cloudflare.com/ajax/libs/gsap/3.5.1/gsap.min.js').then(() => {
          this.loadAnimation();
        });
        this.$forceUpdate();
      }
